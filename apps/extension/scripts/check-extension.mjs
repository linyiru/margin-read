...
assert(
  ...
  "custom content_security_policy is not allowed without security review."
);
...
assert(
  hasMinimalKnownPermissions(manifest),
  "manifest permissions changed; update the allowlist in check-extension.mjs."
);
...
for (const file of files) {
  ...
  assert(
    ...
    `${file} must not load remote scripts.`
  );
...
function containsProviderApiKey(content) {
  ...
  return (
    content.includes("sk-ant-") || content.includes("sk-proj-") || content.includes(googlePrefix)
  );
}
