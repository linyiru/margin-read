...
export function collectUniversalBlocks(
  document: Document,
  ...
  for (const element of Array.from(
    document.querySelectorAll<HTMLElement>(UNIVERSAL_SEMANTIC_SELECTOR)
  )) {
    if (isBrSeparatedContainer(element, options)) {
      blocks.push(
        ...filterCandidateElements(
          splitBrSeparatedContainer(element, document, options),
          "semantic"
        )
      );
      continue;
...
function splitBrSeparatedContainer(
